...
class HtmlLinkExtractionStrategy implements LinkExtractionStrategyInterface
{
    ...
    public function extractLinks(string $content, CrawledPage $crawledPage, string $baseUrl, bool $followExternalLinks): array
    {
        ...
        try {
            ...
            $crawler->filter('a[href]')->each(function (Crawler $node) use ($baseUrl, $crawledPage, $followExternalLinks, &$extractedUrls) {
                $href = $node->attr('href');
                ...
                if ($this->urlNormalizer->isValidUrl($normalizedUrl, $baseUrl, $followExternalLinks)) {
                    $extractedUrls[] = [
                        'url' => $normalizedUrl,
                        'type' => 'link',
                        'depth' => $crawledPage->getDepth() + 1
                    ];
                }
            });

        } catch (\Exception $e) {
            $this->logger->warning('Failed to extract HTML links', [
